...
2. ~~**Rate limiter-ul nu e atomic**~~ — rezolvat. Limita e numărată acum în Redis de `@upstash/ratelimit` (`core/rate-limit/upstash-rate-limiter.ts`), cu fereastră fixă și în mod atomic. La eroare sau timeout de 1 s se trece pe limiterul din memorie. Risc rămas: fallback-ul în memorie nu șterge cheile expirate, deci crește fără limită pe instanțele Fluid Compute cu viață lungă.
3. **CSP-ul** — `unsafe-eval` e trimis acum doar în development (`next.config.ts`). React îl folosește numai acolo, conform `node_modules/next/dist/docs/01-app/02-guides/content-security-policy.md`. `unsafe-inline` rămâne: un nonce ar face toate paginile statice dinamice. Risc rămas: variabilele Custom JavaScript din GTM nu mai rulează în producție.
