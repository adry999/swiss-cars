...
- `readLocaleMessages(locale: string): Promise<MessagesTree | null>` — citește și parse-ază fișierul JSON pentru o locală (protejat de `requireAuth`).
- Tip: `MessagesTree`.

`@features/translations/actions` (Server Actions, protejate de `requireAuth`):
